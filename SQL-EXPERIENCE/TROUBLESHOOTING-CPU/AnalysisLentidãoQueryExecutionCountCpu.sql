/*
    OBJETIVO: Identificar consultas com alto consumo de CPU, I/O e tempo de execução,
              fornecendo métricas detalhadas por execução e total acumulado,
              com foco em objetos do banco e consultas SELECT recentes.
...
/*
    ...
    > Algumas informações de execução da procedure não são consideradas.
      Alguns recursos de temporização como o WAITFOR não são considerados
      nas DMV e aparecem no Profiler.
*/
...
WHERE
    DB_NAME(qt.dbid) <> 'YOUR_DATABASE_DIX'
    AND (
...
-- ============================================================
-- Focado em queries SELECT do banco
...
WHERE
    est.text LIKE 'SELECT%'       -- Filtra apenas SELECTs, se quiser
...
ORDER BY
    eqs.execution_count DESC;         -- eqs.execution_count DESC; eqs.total_worker_time DESC; -- Pode mudar pra total_logical_reads, total_elapsed_time, etc.
