/*
    OBJETIVO: Monitoramento de sessões ativas no SQL Server, identificando
              requisições em execução, sessões bloqueadoras, níveis de
              isolamento de transação, texto e objeto do comando SQL em
              execução, com extração via sys.dm_exec_sql_text.
...
-- Requisições em execução: monitora sessões ativas, bloqueios e comandos SQL
SELECT
    r1.session_id AS SessionId
    ...
    -- Nível de isolamento da transação da sessão bloqueada
    , CASE
    ...
    -- Nome do schema + objeto do comando em execução
    ...
    -- Extrai o texto individual da instrução da sessão bloqueadora
    , SUBSTRING(
        sq2.text,
...
OUTER APPLY sys.dm_exec_sql_text(r1.sql_handle) AS sq1
LEFT OUTER JOIN sys.dm_exec_sessions AS s2
    ON r2.session_id = s2.session_id
OUTER APPLY sys.dm_exec_sql_text(r2.sql_handle) AS sq2
LEFT OUTER JOIN sys.dm_exec_background_job_queue AS jq
    ON jq.session_id = r1.session_id
WHERE 1 = 1
...
ORDER BY
    r1.total_elapsed_time DESC
    , r1.session_id DESC;


-- ============================================================
-- Transações abertas (bloco comentado, desativado)
-- ============================================================
-- SELECT
--     tr.session_id AS SessionId
--     , tr.elapsed_time_seconds AS [TotalElapsedTime (s)]
--     , sess.host_name
--     , sess.program_name
--     , sq.text
-- FROM sys.dm_tran_active_snapshot_database_transactions AS tr
-- INNER JOIN sys.dm_exec_sessions AS sess
--     ON sess.session_id = tr.session_id
-- INNER JOIN sys.sysprocesses AS pr
--     ON pr.spid = tr.session_id
-- CROSS APPLY sys.dm_exec_sql_text(pr.sql_handle) AS sq
-- ORDER BY
