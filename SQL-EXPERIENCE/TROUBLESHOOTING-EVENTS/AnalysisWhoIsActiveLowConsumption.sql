/*
    OBJETIVO: Fornecer versões alternativas da sp_WhoIsActive com baixo
              consumo de recursos, utilizando consultas diretas às DMVs
              sem a sobrecarga da procedure oficial, compatíveis com
              SQL Server 2012+, 2008 e versões anteriores.
...
-- ============================================================
-- Versão completa (SQL Server 2012+)
-- ============================================================
...
SELECT
    ...
    + RIGHT('000' + CAST(DATEDIFF(SECOND, COALESCE(B.start_time, A.login_time), GETDATE()) AS VARCHAR), 3)             AS Duration
  ...
  , A.login_name
  , '(' + CAST(COALESCE(E.wait_duration_ms, B.wait_time) AS VARCHAR(20)) + 'ms)'
    + COALESCE(E.wait_type, B.wait_type)
    + COALESCE
      (
          CASE
              WHEN COALESCE(E.wait_type, B.wait_type) LIKE 'PAGE%LATCH%'
              THEN ':' + DB_NAME(LEFT(E.resource_description, CHARINDEX(':', E.resource_description) - 1))
                   + ':' + SUBSTRING(E.resource_description, CHARINDEX(':', E.resource_description) + 1, 999)
              WHEN COALESCE(E.wait_type, B.wait_type) = 'OLEDB'
              THEN '[' + REPLACE(REPLACE(E.resource_description, ' (SPID=', ':'), ')', '') + ']'
              ELSE ''
          END,
          ''
      )                                                                                                                 AS wait_info
  ...
  , FORMAT
    (
        ...
        (
            CASE
                WHEN F.tempdb_allocations > F.tempdb_current
                THEN F.tempdb_allocations - F.tempdb_current
                ELSE 0
            END,
            0
        ),
  ...
  , (CASE
        ...
        WHEN B.deadlock_priority > -5 AND B.deadlock_priority < 5 THEN 'Normal'
  ...
  , CASE
        ...
        THEN 'SQLAgent - TSQL Job (' + D.name + ' - ' + SUBSTRING(A.program_name, 67, LEN(A.program_name) - 67) + ')'
        ELSE A.program_name
    END                                                                                                                 AS program_name
...
FROM
    ...
    INNER JOIN sys.dm_exec_connections AS C WITH (NOLOCK)
        ON A.session_id = C.session_id
           AND A.endpoint_id = C.endpoint_id
        ...
        SELECT
          ...
          , ROW_NUMBER() OVER
            (
                ...
                ORDER BY
                    CASE WHEN wait_type LIKE 'PAGE%LATCH%' THEN 0 ELSE 1 END
                  , wait_duration_ms
            )                                                                  AS Ranking
        FROM
    ...
    LEFT JOIN sys.dm_resource_governor_workload_groups AS H
...
-- ============================================================
-- Versão mais rápida (SQL Server 2012+, com menos recursos)
-- ============================================================
...
SELECT
  ...
  , '(' + CAST(COALESCE(E.wait_duration_ms, B.wait_time) AS VARCHAR(20)) + 'ms)'
    ...
    + COALESCE
      (
          CASE
              WHEN COALESCE(E.wait_type, B.wait_type) LIKE 'PAGEIOLATCH%'
              THEN ':' + DB_NAME(LEFT(E.resource_description, CHARINDEX(':', E.resource_description) - 1))
                   + ':' + SUBSTRING(E.resource_description, CHARINDEX(':', E.resource_description) + 1, 999)
              WHEN COALESCE(E.wait_type, B.wait_type) = 'OLEDB'
              THEN '[' + REPLACE(REPLACE(E.resource_description, ' (SPID=', ':'), ')', '') + ']'
        ...
        SELECT
          ...
          , ROW_NUMBER() OVER
            (
                PARTITION BY session_id
                ORDER BY
                    CASE WHEN wait_type LIKE 'PAGEIO%' THEN 0 ELSE 1 END
                  , wait_duration_ms
            )                                                                  AS Ranking
        FROM
            sys.dm_os_waiting_tasks
...
-- ============================================================
-- Versão para SQL Server 2008 e anteriores
-- ============================================================
