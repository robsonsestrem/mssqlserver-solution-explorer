/*
    OBJETIVO: Identificar a data/hora da última reinicialização da instância
              do SQL Server e do SQL Agent, além de analisar o histórico
              de reinicializações do SQL Agent para identificar padrões
              como dias e horários mais frequentes.
...
-- ============================================================
-- Última reinicialização da instância SQL Server
-- ============================================================
...
-- ============================================================
-- SQL Server Agent - Histórico de reinicializações
-- ============================================================
...
-- ============================================================
-- Análise de padrões de reinicialização do SQL Agent
-- ============================================================
...
INTO
    #Dados
FROM
    msdb.dbo.syssessions AS A
    INNER JOIN msdb.dbo.syssessions AS B
        ON A.session_id = B.session_id + 1;

IF (OBJECT_ID('tempdb..#Dia_Mais_Inicializado') IS NOT NULL)
    DROP TABLE #Dia_Mais_Inicializado;

SELECT
    Dia
  , COUNT(*)                                                                 AS Quantidade
INTO
    #Dia_Mais_Inicializado
FROM
    #Dados
GROUP BY
    Dia;

IF (OBJECT_ID('tempdb..#Hora_Mais_Inicializada') IS NOT NULL)
    DROP TABLE #Hora_Mais_Inicializada;

SELECT
    Hora
  , COUNT(*)                                                                 AS Quantidade
INTO
    #Hora_Mais_Inicializada
FROM
    #Dados
GROUP BY
    Hora;

IF (OBJECT_ID('tempdb..#Dia_Semana_Mais_Inicializado') IS NOT NULL)
    DROP TABLE #Dia_Semana_Mais_Inicializado;

SELECT
    Dia_Semana
  , COUNT(*)                                                                 AS Quantidade
INTO
    #Dia_Semana_Mais_Inicializado
FROM
    #Dados
GROUP BY
    Dia_Semana;

DECLARE @Qt_Media_Dias_Entre_Inicializacoes INT = (
    SELECT AVG(Qt_Diferenca)
    FROM #Dados
);
