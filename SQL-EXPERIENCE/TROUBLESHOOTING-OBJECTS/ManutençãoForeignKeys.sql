/*
    OBJETIVO:    Gerar dinamicamente scripts de DROP e CREATE de foreign keys
                 (simples e compostas) para um banco, tabela ou schema parametrizáveis.
    PROJETO:     mssqlserver-solution-explorer
    REFERÊNCIA:  https://www.dirceuresende.com/blog/como-identificar-apagar-e-recriar-foreign-keys-fk-de-uma-tabela-no-sql-server/
*/

-- ---------------------------------------------------------------------------
-- Procedure 1: stpRecria_FK_Tabela — DROP e CREATE de FKs por tabela/schema
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    -- Monta o script de DROP das FKs
    SET @query = '
    SELECT
    ...
    -- -----------------------------------------------------------------------
    -- Monta o script de RECREATE (CREATE) das FKs
    -- -----------------------------------------------------------------------
    SET @query = '
    SELECT
    ...
    FROM
        ...
        JOIN ' + @Database + '.sys.objects             AS objeto       ON fk.parent_object_id = objeto.object_id
        JOIN ' + @Database + '.sys.columns             AS colunas      ON fc.parent_column_id = colunas.column_id AND fk.parent_object_id = colunas.object_id
        JOIN ' + @Database + '.sys.schemas             AS schema_ori   ON objeto.schema_id = schema_ori.schema_id
        JOIN ' + @Database + '.sys.objects             AS objeto_ref   ON fc.referenced_object_id = objeto_ref.object_id
        JOIN ' + @Database + '.sys.columns             AS colunas_ref  ON fc.referenced_column_id = colunas_ref.column_id AND fk.referenced_object_id = colunas_ref.object_id
        JOIN ' + @Database + '.sys.schemas             AS schema_ref   ON objeto_ref.schema_id = schema_ref.schema_id';
...
-- ---------------------------------------------------------------------------
-- Procedure 2: stpRecria_FK — DROP e CREATE de FKs simples com WITH(NOLOCK)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Procedure 3: stpRecria_FK_Composta — DROP e CREATE de FKs compostas (CTE)
-- ---------------------------------------------------------------------------
