/*
 *
	OBJETIVO: Template de procedure para tratamento padronizado de erros e transações no SQL Server,
			  com verificação do estado da transação (XACT_STATE) e relançamento do erro via THROW.
 ...
 */
-- ============================================================
-- GLPI 1105 - Rotina plano parcial: Otimização de código
-- ============================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Teste para validação de erro (substituir 0 para simular divisão por zero)
        SELECT 100 / 20; -- teste pra erro: 100/0

        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        -- Verifica o estado da transação e executa rollback ou commit conforme necessário
        IF (XACT_STATE()) = -1
        BEGIN
        ...
        SELECT @ErrorMessage =
               N'PROCEDURE: SP_PLANO_PARCIAL; - Erro na linha '
