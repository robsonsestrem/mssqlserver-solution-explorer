/*
    OBJETIVO: Scripts de diagnóstico para listagem de triggers DML e configuração
              da ordem de execução (FIRST/LAST) via sp_settriggerorder nas tabelas
              ACSPA, AVALS, CNSUL, MEDRS, PSSOA, QTNAV e RESPC.
    PROJETO: mssqlserver-solution-explorer
*/
-- ============================================================
-- SEÇÃO 1: DIAGNÓSTICO — Listagem de triggers e propriedades
-- ============================================================

-- Exibe propriedades dos triggers DML por tabela: tipo de evento, estado e owner
SELECT
...
-- ============================================================
-- SEÇÃO 2: DIAGNÓSTICO — Ordem de execução dos triggers
-- ============================================================

-- Verifica posição de execução (FIRST/LAST) dos triggers por tipo de evento DML
SELECT
...
-- Visão consolidada: triggers, tipo de evento e datas de criação e modificação
SELECT
     sys.TABLES.name              AS table_name
    ,sys.TRIGGERS.name            AS trigger_name
    ,sys.TRIGGER_EVENTS.type      AS event_type_code
    ,sys.TRIGGER_EVENTS.TYPE_DESC AS event_type_desc
    ,IS_FIRST
    ,IS_LAST
...
/*
 * Tabelas monitoradas:
 * ACSPA | AVALS | CNSUL | MEDRS | PSSOA | QTNAV | RESPC
 *
 * FONTES:
...
-- ============================================================
-- SEÇÃO 3: CONFIGURAÇÃO — Ordem de execução via sp_settriggerorder
-- Objetivo: Garantir sequência correta e evitar operações desnecessárias
-- ============================================================
...
BEGIN
    EXEC sys.sp_settriggerorder
        @triggername = 'TGR_EXCLUI_ACSPA_INFO',
        @order       = 'FIRST',
        @stmttype    = 'DELETE';
END;
GO
...
BEGIN
    EXEC sys.sp_settriggerorder
        @triggername = 'TG_NAO_DELETAR_AVALS_CONCLUIDAS',
        @order       = 'FIRST',
        @stmttype    = 'DELETE';
END;
GO
...
BEGIN
    EXEC sys.sp_settriggerorder
        @triggername = 'TG_VERIFICA_AGENDAMENTO_NA_DATA',
        @order       = 'FIRST',
        @stmttype    = 'INSERT';
END;
GO
...
BEGIN
    EXEC sys.sp_settriggerorder
        @triggername = 'TGR_CALCULA_RISCO_MEDRS',
        @order       = 'LAST',
        @stmttype    = 'INSERT';
...
BEGIN
    EXEC sys.sp_settriggerorder
        @triggername = 'TG_NAO_DELETAR_QTNAV_AVALS_CONCLUIDAS',
        @order       = 'FIRST',
        @stmttype    = 'DELETE';
END;
GO
...
BEGIN
    EXEC sys.sp_settriggerorder
        @triggername = 'TG_NAO_INSERIR_RESPC_AVALS_APOS_CONCLUIDA',
        @order       = 'FIRST',
        @stmttype    = 'INSERT';
END;
GO
...
BEGIN
    EXEC sys.sp_settriggerorder
        @triggername = 'TGR_CALCULA_RISCO_RESPC',
        @order       = 'LAST',
        @stmttype    = 'INSERT';
