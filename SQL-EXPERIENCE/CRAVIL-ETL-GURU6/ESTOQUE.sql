-----------------------------------------------------------------------------------------------------------------
-- Usado instrução abaixo para realizar inserções dos resultados numa tabela como histórico
-----------------------------------------------------------------------------------------------------------------
declare @posicao datetime = '20170917'	   
...
select distinct
   ...
   , t1.Estoque
   , t1.CustoTotal
 from IntegraTICravil.Bi.HistoricoCMV as t1 
 ...
 where t1.DataEmissao = @posicao

 UNION ALL

	select x.FilCod, t7.FilNom, @posicao as Posicao, x.ProCod, t3.ProNom
	...
	) as x cross apply GesCooper90.dbo.GetCustoMercadoria(x.FilCod, x.ProCod, @posicao) as t2
	--
...
--select * from IntegraTICravil.Bi.HistoricoPosicaoEstoque
--where DataEmissao = '20170920'