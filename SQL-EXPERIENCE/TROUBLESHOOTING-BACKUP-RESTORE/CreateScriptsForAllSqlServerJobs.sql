/*
    OBJETIVO: Gerar scripts de backup para todos os jobs do SQL Server Agent,
              incluindo steps, schedules, operadores, alertas e notificações,
              utilizando xp_cmdshell para salvar os scripts em arquivos de texto.
...
-- ============================================================
-- Habilitar xp_cmdshell (necessário para salvar arquivos)
-- ============================================================
...
-- ============================================================
-- Procedure para backup de um job específico
-- ============================================================
...
BEGIN
    ...
    -- ============================================================
    -- Backup do cabeçalho do job (sysjobs)
    -- ============================================================
    SELECT
    ...
    -- ============================================================
    -- Backup dos steps do job (sysjobsteps)
    -- ============================================================
    ...
    SELECT
        JobSteps.*
    FROM
        msdb.dbo.sysjobsteps AS JobSteps
        INNER JOIN msdb.dbo.sysjobs AS Job
            ON Job.job_id = JobSteps.job_id
    WHERE
        Job.name = @JobName;
    ...
    FETCH NEXT FROM Cur
    INTO @job_id, @step_id, @step_name, @subsystem, @command, @flags,
         @additional_parameters, @cmdexec_success_code, @on_success_action,
         @on_success_step_id, @on_fail_action, @on_fail_step_id, @server,
         @database_name, @database_user_name, @retry_attempts, @retry_interval,
         @os_run_priority, @output_file_name, @last_run_outcome,
         @last_run_duration, @last_run_retries, @last_run_date,
         @last_run_time, @proxy_id, @step_uid;
    ...
    BEGIN
        SET @JobCommand = ' INSERT INTO [msdb].[dbo].[sysjobsteps] '
            ...
            + ' VALUES ( ''' + ISNULL(CONVERT(VARCHAR(MAX), @job_id), 'Null') + ''','
            + ISNULL(CONVERT(VARCHAR(MAX), @step_id), 'Null') + ','
            + +'''' + ISNULL(CONVERT(VARCHAR(MAX), @step_name), 'Null')
            + ''',' + '''' + ISNULL(CONVERT(VARCHAR(MAX), @subsystem), 'Null')
            + ''',' + '''';

        SET @JobCommand1 = ISNULL(CONVERT(VARCHAR(MAX), @command), 'Null') + ''',';
        SET @JobCommand1 = REPLACE(@JobCommand1, CHAR(13) + CHAR(10), ' ');

        SET @JobCommand2 = ISNULL(CONVERT(VARCHAR(MAX), @flags), 'Null') + ',' + ''''
            + ISNULL(CONVERT(VARCHAR(MAX), @additional_parameters), 'Null') + ''','
            + ISNULL(CONVERT(VARCHAR(MAX), @cmdexec_success_code), 'Null') + ','
            + ISNULL(CONVERT(VARCHAR(MAX), @on_success_action), 'Null')
            + ',' + ISNULL(CONVERT(VARCHAR(MAX), @on_success_step_id), 'Null') + ','
            + ISNULL(CONVERT(VARCHAR(MAX), @on_fail_action), 'Null')
            + ',' + ISNULL(CONVERT(VARCHAR(MAX), @on_fail_step_id), 'Null') + ','
            + ISNULL(CONVERT(VARCHAR(MAX), @server), 'Null') + ','
            + +'''' + ISNULL(CONVERT(VARCHAR(MAX), @database_name), 'Null') + ''',' + +''''
            + ISNULL(CONVERT(VARCHAR(MAX), @database_user_name), 'Null') + ''','
            + ISNULL(CONVERT(VARCHAR(MAX), @retry_attempts), 'Null')
            + ',' + ISNULL(CONVERT(VARCHAR(MAX), @retry_interval), 'Null') + ','
            + ISNULL(CONVERT(VARCHAR(MAX), @os_run_priority), 'Null')
            + ',' + +'''' + ISNULL(CONVERT(VARCHAR(MAX), @output_file_name), 'Null')
            + ''',' + ISNULL(CONVERT(VARCHAR(MAX), @last_run_outcome), 'Null') + ','
            + ISNULL(CONVERT(VARCHAR(MAX), @last_run_duration), 'Null')
            + ',' + ISNULL(CONVERT(VARCHAR(MAX), @last_run_retries), 'Null') + ','
            + ISNULL(CONVERT(VARCHAR(MAX), @last_run_date), 'Null')
            + ',' + ISNULL(CONVERT(VARCHAR(MAX), @last_run_time), 'Null') + ','
            + ISNULL(CONVERT(VARCHAR(MAX), @proxy_id), 'Null') + ','
            + +'''' + ISNULL(CONVERT(VARCHAR(MAX), @step_uid), 'Null') + '''' + ' )';

        INSERT INTO @OutPut (Echo) VALUES (@JobCommand);
        INSERT INTO @OutPut (Echo) VALUES (@JobCommand1);
        INSERT INTO @OutPut (Echo) VALUES (@JobCommand2);
    ...
    -- ============================================================
    -- Backup dos schedules (sysschedules e sysjobschedules)
    -- ============================================================
    SET @JobCommand = '';
    ...
    SELECT
      ...
      , Comand_sysschedules =
            ...
            + 'VALUES ( ' + '''' + CONVERT(VARCHAR(MAX), Sch.schedule_uid) + ''',' + +''''
    ...
    FROM
        msdb.dbo.sysjobschedules AS JobSch
        INNER JOIN msdb.dbo.sysjobs AS Job
            ON Job.job_id = JobSch.job_id
        INNER JOIN msdb.dbo.sysschedules AS Sch
            ON JobSch.schedule_id = Sch.schedule_id
    WHERE
        Job.name = @JobName;

    OPEN Cur1;

    FETCH NEXT FROM Cur1 INTO @Command_sysjobschedules, @Comand_sysschedules;

    WHILE (@@FETCH_STATUS = 0)
    BEGIN
        INSERT INTO @OutPut (Echo) VALUES (@Comand_sysschedules);
        INSERT INTO @OutPut (Echo) VALUES (@Command_sysjobschedules);
    ...
    -- ============================================================
    -- Backup de operadores, alertas e notificações
    -- ============================================================
    SET @JobCommand = '';
    ...
    SELECT
      ...
      , Command_sysalerts =
            ...
            + ISNULL(CONVERT(VARCHAR(MAX), Ale.[event_category_id]), 'Null') + ','
            ...
            + +') Set @Al = @@IDENTITY '
    FROM
        msdb.dbo.sysalerts AS Ale
        INNER JOIN msdb.dbo.sysnotifications AS Noti
            ON Ale.id = Noti.alert_id
        INNER JOIN msdb.dbo.sysoperators AS Ope
            ON Noti.operator_id = Ope.id
        INNER JOIN msdb.dbo.sysjobs AS Job
            ON Job.job_id = Ale.job_id
    WHERE
        Job.name = 'aaJob';

    OPEN Cur2;

    FETCH NEXT FROM Cur2 INTO @Command_notifications, @Command_operators, @Command_sysalerts;
    ...
    WHILE (@@FETCH_STATUS = 0)
    BEGIN
        INSERT INTO @OutPut (Echo) VALUES (@Command_operators);
        INSERT INTO @OutPut (Echo) VALUES (@Command_sysalerts);
        INSERT INTO @OutPut (Echo) VALUES (@Command_notifications);
    ...
    -- ============================================================
    -- Gerar arquivo de script via xp_cmdshell
    -- ============================================================
    ...
    FETCH NEXT FROM Cur3 INTO @EchoField;

    WHILE (@@FETCH_STATUS = 0)
    BEGIN
        SET @EchoCommand = 'ECHO ' + @EchoField + ' >> ' + @Path + @JobName
            + REPLACE(REPLACE(CONVERT(VARCHAR(200), GETDATE()), ' ', ''), ':', '') + '.txt';
...
-- ============================================================
-- Procedure para backup de todos os jobs
-- ============================================================
