...
BEGIN 
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			DECLARE @vSubject NVARCHAR(255) = 'RELAT�RIO DE NF-e N�O AUTORIZADAS PELO SEFAZ'
			...
			DECLARE
				...
				select 
						...
						case m.NfSituacao
							...
							when 8 then 'NF-e � Cancelar'
							when 9 then 'Ag. Confer�ncia'
							when 10 then 'Aguardando Armazenagem'
							when 11 then 'Aguardando Autoriza��o'
							when 12 then 'Aguardando Libera��o'
							when 13 then 'Aguardando Processamento'
						else 'Indefinida'
						end as Situa��o,		
						case 
							when m.NfeChNfe = '' then 'N�o Gerada'
							else isnull(m.NfeChNfe,'N�o Gerada')			
						END AS Chave,
				...
				-- traz s� aquelas que cont�m n� de NFE	conforme a situa��o.
				and 
				( (m.NfTipDoc = 'nfe' and m.NfSituacao not in(10,11,12,13)) or (m.NfTipDoc <> 'nfe' and m.NfSituacao in (10,11,12,13)) )
												   
				and p.OpeDocCod = 1		-- s� opera��es de nota eletr�nica que precisa autoriza��o	
			-------------------------------------------------------------------------------------------------------------------------------------------------------------	
				...
				ELSE	-- caso tenha registros monta numa tabela
					SET @vBody = 																 -- Abre tabela - Nome das colunas (HTML)
						'
							<h3><font color=black bold=true size= 4> Rela��o de NF-e n�o autorizadas </font><td align=center> </h3>
								<table cellpadding=2 cellspacing=1 border=3 align=center>
									<tr>
										...
										<th bgcolor=#0B0B61 width=200> <font color=white> Situa��o </font></th>
						...
						BEGIN
							...
							(
								SELECT
									...
									'<td>'+n.DataHora+'</td>'+						 -- inser��o de dados a partir da 2� linha (HTML)
					...
					SET @vBody = @vBody + '</table><br>';													 -- Fecha 2� tabela (HTML)			
