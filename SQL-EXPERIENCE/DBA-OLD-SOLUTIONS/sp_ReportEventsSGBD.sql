...
AS
	BEGIN
		...
		SELECT
			...
			INTO #Tabela;																	-- inser��o de dados em tabela tempor�ria
																							-- Abre 1� tabela (HTML)
																							-- colunas da primeira linha - t�tulos (HTML)
		SET @vBody =
		...
		INSERT INTO @LogSql1 exec sp_readerrorlog;											 -- inser��o de dados na vari�vel tipo table
		...
		INSERT INTO @LogSQL2
			SELECT TOP 30
				LogDate, ProcessInfo, Text													 -- inser��o de dados em outra vari�vel do tipo table
			FROM @LogSQL1 l
			WHERE l.Text LIKE '%erro%'										
			ORDER BY 1 DESC;

		SET @Qt = @@ROWCOUNT;
		SET @vBody = @vBody +																 -- Abre 2� tabela - t�tulos (HTML)
			'<br><br>
			...
			BEGIN
				...
				(
					SELECT
						...
						'<td>'+CONVERT(VARCHAR,ProcessInfo)+'</td>'+						 -- inser��o de dados a partir da 2� linha (HTML)
		...
		INSERT INTO @LogSQL3
			...
			FROM @LogSQL1 l																	 -- inser��o de dados em vari�vel do tipo table
			ORDER BY 1 DESC;

		SET @Qt = @@ROWCOUNT;
		SET @Loop = 1;	
		SET @vBody = @vBody +																 -- Abre 3� tabela - t�tulos (HTML)
			'<br><br>
		...
		EXEC master.dbo.xp_fixeddrives;														 -- inser��o de dados em var�vel table

		SET @Qt = @@ROWCOUNT;
		SET @Loop = 1;

		SET @vBody = @vBody +																 -- Abre 4� tabela - t�tulos (HTML)
			'<br><br>
		...
		WHILE @Loop <= @Qt
			BEGIN
				...
				(
					SELECT
						'<tr>'+
						'<td>'+CONVERT(VARCHAR,t.drive_letter)+'</td>'+						-- inser��o de dados a partir da 2� linha (HTML)
		...
		SELECT
			t.Banco
			, t.ArquivoDeDados_EspacoReservadoEmDisco_MB									 -- inser��o de dados na vari�vel do tipo table
		...
		SET @vBody = @vBody +																  -- Abre 5� tabela - t�tulos (HTML)
			'<br><br>
				<h3>Tamanho dos Bancos de Dados de Usu�rio</h3>
					<table border=1 cellpadding=2>											
						<tr>
							<th>Banco</th>
							<th>Arquivo de Dados - Espa�o Reservado em Disco</th>
							<th>Espa�o Usado (dados)</th>
							<th>Espa�o Livre (dados)</th>
							<th>Arquivo de Log - Espa�o Reservado em Disco</th>
							<th>Espa�o Usado (log)</th>
							<th>Espa�o Livre (log)</th>
						</tr>';
		...
		WHILE @Loop <= @Qt
			BEGIN
				...
				(
					SELECT
						'<tr>'+																 -- inser��o de dados a partir da 2� linha (HTML)
						'<td>'+ Banco +'</td>'+
		...
		SET @vBody = @vBody +																 -- Abre 6� tabela (HTML)
			'<br><br>
				<h3>Espa�o no banco de dados interno TempDB</h3>
					<table border=1 cellpadding=2>											 							
						<tr>
							<th>P�ginas Livres (tamanho por p�gina � 8Kb sendo 128 P�g./Mb)</th>
							<th>Espa�o Livre</th>
						</tr>';
			...
			(
				SELECT
					'<tr>'+																	 -- inser��o de dados na segunda linha (HTML)
		...
		SET @vBody = @vBody +																 -- Abre 7� tabela - t�tulos (HTML)
			'<br><br>
		...
		INSERT INTO @Backups																 -- inser��o de dados na vari�vel do tipo table
			select
				s.database_name
				, s.server_name
				, ISNULL(s.name,'')
		...
		INSERT INTO @Jobs																    -- inser��o de dados numa vari�vel do tipo table
		SELECT
			j.name
			, (CASE h.run_status
				WHEN 0 THEN 'Falha'
		...
		WHILE @Loop <= @Qt
			BEGIN
				...
				select
					...
					'<td>'+CONVERT(VARCHAR(50),j.status)+'</td>'+							-- inser��o de dados a partir da 2� linha (HTML)
