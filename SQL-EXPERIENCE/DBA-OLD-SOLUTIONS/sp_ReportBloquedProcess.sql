...
CREATE OR ALTER PROCEDURE Management.[sp_ReportBloquedProcess] @ExibirApenasHtml BIT = 0	-- Caso par�metro for 1, vai me trazer resultado com o script HTML.
WITH ENCRYPTION
AS
BEGIN
		SET NOCOUNT ON;			
		...
		DECLARE @vSubject NVARCHAR(255) = 'Relat�rio di�rio - Processos Bloqueados no Sistema'
		...
		-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------		
						...
						BEGIN 
							set @vBody = '<table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:12px>
											<tr height=20  style=color:black;>
												<td width=300 style=height:20.0pt>N�o houve processos bloqueados.
					...
					ELSE
							BEGIN	-- caso tenha registros monta numa tabela
									SET @vBody = '<table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:12px>
													<tr height=20  style=color:black;>
														<td width=300 style=height:20.0pt>Anexo dados de processos bloqueados dispon�veis para an�lise.
																						<br>Os dados coletados s�o apenas de bloqueios que duraram mais de 10 segundos.																		
																						<br>Data dos eventos: '+CONVERT(VARCHAR(12),@inicio,105)+'
																						<br>Inst�ncia: '+@@SERVERNAME+'
														</td>
													</tr>
