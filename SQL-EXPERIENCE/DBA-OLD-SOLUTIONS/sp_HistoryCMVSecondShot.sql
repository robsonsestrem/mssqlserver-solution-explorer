...
begin
	...
	BEGIN TRY
		BEGIN TRANSACTION 	
		    set @resultSetHistory = (select COUNT(*) from IntegraTICravil.Bi.HistoricoCMV as t1
										 where t1.DataEmissao >= @datainicio) -- data de movimento		
			--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			-- Condicional para n�o enviar e-mail, pois a carga foi realizada com sucesso
			--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			IF(@resultSetHistory > 0)			
			BEGIN
				print 'INTEGRA��O J� FOI REALIZADA COM SUCESSO!'
			END

			--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			-- Condicional para tentar realizar carga novamente
			--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			IF(@resultSetHistory = 0)
			BEGIN
				SET @resultSet = (
				...
				SET @totalVendas = (SELECT CAST(ISNULL(IntegraTICravil.Erp.fn_FormatIntToMoney( IntegraTICravil.Bi.fn_TotaisEmailBi(@datainicio, @datafinal, 1) ), 0) AS VARCHAR(20)))

				--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				-- Condicional para alertar falha de carga dos dados e mostrar o que n�o foi integrado
				--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				IF(@resultSet > 0 and datepart(hour, getdate()) < 10)
					BEGIN		
								SET @totalCupons = (SELECT CAST(ISNULL(IntegraTICravil.Erp.fn_FormatIntToMoney( IntegraTICravil.Bi.fn_TotaisEmailBi(@datainicio, @datafinal, 3) ), 0) AS VARCHAR(20)))						
								SET @CorpoEmail = '
						...
						END
				--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				-- Condicional para efetuar a carga com ou sem falhas
				--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				IF(@resultSet = 0 or (@resultSet > 0 and datepart(hour, GETDATE()) >= 10))
					BEGIN
						...
						if(@resultSet > 0)
						begin
						 set @texto = 'Integra��o de hoje das Receitas com CMV foram realizadas parcialmente, pois ainda existem cupons n�o integrados.'
						end
						if(@resultSet = 0)
						begin 
						 set @texto = 'Integra��o de hoje das Receitas com CMV realizada com sucesso.'
						end
						...
						SET @totalCusto = (SELECT CAST(ISNULL(IntegraTICravil.Erp.fn_FormatIntToMoney( IntegraTICravil.Bi.fn_TotaisEmailBi(@datainicio, @datafinal, 2) ), 0) AS VARCHAR(20)))
