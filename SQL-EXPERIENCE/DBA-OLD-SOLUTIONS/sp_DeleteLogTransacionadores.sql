	...
	BEGIN TRY 
		...
		begin transaction
			...
			(select count(x.Registros)
				...
				from IntegraTICravil.LogErp.TransacionadorLogDML as t1
			...
			WHILE( @qtdadeDias > @qtdadeManterDias ) 
			  BEGIN 
				  set @dataMin = 
				  (
					  select cast(dateadd(day, 1 ,((select min(t1.DateDML) from IntegraTICravil.LogErp.TransacionadorLogDML as t1))) as date)
				  )
				  
				  DELETE FROM IntegraTICravil.LogErp.TransacionadorLogDML
