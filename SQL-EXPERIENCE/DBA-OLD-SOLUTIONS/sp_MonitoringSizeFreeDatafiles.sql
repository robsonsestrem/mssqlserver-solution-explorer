	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			INSERT INTO ##tempDatafileFree(DatabaseName , 
			  ...
			  --where DatabaseName like '%YOUR_DATABASE%'	-- Tratado para quando houver uma base que n�o precisa monitorar


			  ---------------------------------------------------------------------------------------------------------------------------------------------------
			  -- Valida as condi��es para o envio de e-mail
			  ---------------------------------------------------------------------------------------------------------------------------------------------------
			  IF((SELECT count(*) FROM ##tempDatafileFree AS t WHERE t.PercFreeFile <= @percentFree) > 0)
				BEGIN
					DECLARE 				
						@Assunto VARCHAR(200) = @@SERVERNAME + ' - Monitoramento de Espa�o Livre nos DataFiles',
						@Destinatario VARCHAR(MAX) = 'suporte@cravil.com.br',
						@Mensagem VARCHAR(MAX)
            		      
					SET @Mensagem = '
					Aten��o DBA,<br>
					Espa�o livre em algum(s) arquivos de dados (mdf, ndf e ldf) est� reduzido (menor que 5%).
