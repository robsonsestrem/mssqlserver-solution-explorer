/*
 *
    OBJETIVO: Procedure para geração e envio de relatório diário de checklist do SQL Server,
              incluindo informações de versão, recursos, serviços, espaço em disco,
              status dos databases, tamanhos de arquivos, configurações de crescimento,
              contagem de VLF e outras métricas de saúde do servidor.
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               ...
               '<td height=20 style=height:15.0pt>' + ISNULL(CASE SERVERPROPERTY('IsClustered') WHEN 1 THEN 'Sim' ELSE 'Não' END, '') + '</td>' +
        ...
        -- ============================================================
        -- Informações da instância (serviços)
        -- ============================================================
        SET @des_MensagemHTML = @des_MensagemHTML + '<br>
        <table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:14px>
            <tr height=20 align=center style=color: #FFFFFF; background: #44546A;>
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   WHEN CAST(ROW_NUMBER() OVER(ORDER BY [servicename] ASC) % 2 AS BIT) = 1
                       THEN '<tr height=20 align=center style=height:15.0pt>'
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   WHEN CAST(ROW_NUMBER() OVER(ORDER BY d.volume_mount_point ASC) % 2 AS BIT) = 1
                       THEN '<tr height=20 align=center style=height:15.0pt>'
                   ELSE '<tr height=20 align=center style=height:15.0pt; background: #E4E4E4;>'
               END +
               '<td height=20 style=height:15.0pt>' + d.volume_mount_point + '</td>' +
               '<td height=20 style=height:15.0pt>' + CAST(d.EspacoTotal_Gb AS VARCHAR(20)) + '</td>' +
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   WHEN CAST(ROW_NUMBER() OVER(ORDER BY t1.DateShutdown DESC) % 2 AS BIT) = 1
                       THEN '<tr height=20 align=center style=height:15.0pt>'
                   ELSE '<tr height=20 align=center style=height:15.0pt; background: #E4E4E4;>'
               END +
        ...
        -- ============================================================
        -- Informações dos Databases - Título
        -- ============================================================
        SET @des_MensagemHTML = @des_MensagemHTML + '
        <table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:20px>
            <tr height=20>
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   WHEN CAST(ROW_NUMBER() OVER(ORDER BY [MST].[name] ASC) % 2 AS BIT) = 1
        ...
        FROM [sys].[databases] AS [MST]
            ...
            (
                ...
                GROUP BY [B].[name]
            ) AS [AA] ON [AA].[LOG_DBNAME] = [MST].[name]
        ORDER BY [MST].[name]
        ...
        -- ============================================================
        -- Totais em Arquivos (mdf, ndf) por Database
        -- ============================================================
        SET @des_MensagemHTML = @des_MensagemHTML + '<br>
        <table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:18px>
            <tr height=20 style=color: #FFFFFF; background: #44546A;>
                <td height=20 colspan=7 style=height:20.0pt;text-align:center>Totais em Arquivos (mdf, ndf) por Database</td>
            </tr>
        </table>'
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   WHEN CAST(ROW_NUMBER() OVER(ORDER BY t.Banco ASC) % 2 AS BIT) = 1
                       THEN '<tr height=20 align=left style=height:15.0pt>'
        ...
        -- ============================================================
        -- Tamanho dos Arquivos dos Bancos de Dados e Percentual Livre
        -- ============================================================
        SET @des_MensagemHTML = @des_MensagemHTML + '<br>
        <table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:18px>
            <tr height=20 style=color: #FFFFFF; background: #44546A;>
                <td height=20 colspan=7 style=height:20.0pt;text-align:center>Tamanho dos Arquivos dos Bancos de Dados e Percentual Livre dos Mesmos</td>
            </tr>
        </table>'
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   WHEN t1.PercFree <= 5.00
                       THEN '<tr height=20 style=height:15.0pt;background: #FFFF00>'
                   ELSE
                       CASE
                           WHEN CAST(ROW_NUMBER() OVER(ORDER BY t1.DatabaseName, Name ASC) % 2 AS BIT) = 1
                               THEN '<tr height=20 style=height:15.0pt>'
                           ELSE '<tr height=20 style=height:15.0pt; background: #E4E4E4;>'
                       END
               END +
        ...
        <table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:18px>
            <tr height=20 style=color: #FFFFFF; background: #44546A;>
                <td height=20 colspan=7 style=height:20.0pt;text-align:center>Configurações de Crescimentos/Limites em Cada Datafile</td>
            </tr>
        </table>'
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   WHEN m.[Limite_Usado_%] >= 90.00
                       THEN '<tr height=20 style=height:15.0pt;background: #FFFF00>'
                   ELSE
                       CASE
                           WHEN CAST(ROW_NUMBER() OVER(ORDER BY m.Databases ASC) % 2 AS BIT) = 1
                               THEN '<tr height=20 style=height:15.0pt>'
                           ELSE '<tr height=20 style=height:15.0pt; background: #E4E4E4;>'
                       END
               END +
        ...
        -- ============================================================
        -- Total de Arquivos VLF em Cada Arquivo de Log
        -- ============================================================
        SET @des_MensagemHTML = @des_MensagemHTML + '<br>
        <table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:18px>
            <tr height=20 style=color: #FFFFFF; background: #44546A;>
                <td height=20 colspan=7 style=height:20.0pt;text-align:center>Total de Arquivos VLF (virtual log files) em Cada Arquivo de Log dos Banco de Dados</td>
            </tr>
        </table>'
        ...
        SELECT @des_MensagemHTML = @des_MensagemHTML +
               CASE
                   ...
                   ELSE
                       CASE
                           WHEN CAST(ROW_NUMBER() OVER(ORDER BY [num_of_rows] DESC) % 2 AS BIT) = 1
                               THEN '<tr height=20 style=height:15.0pt>'
                           ELSE '<tr height=20 style=height:15.0pt; background: #E4E4E4;>'
                       END
               END +
