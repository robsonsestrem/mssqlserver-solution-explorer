...
-- habilitar Service Broker e Trustworthy
-- ============================================================
USE [YOUR_DATABASE]
GO
...
-- ============================================================
-- Automatizando o processo caso 
-- tenha problema de mesmo ID do Service Broker
-- ============================================================
...
(
    SELECT
        ...
        , [Database_id]     = [MainLock].[Process].value('@dbid', 'INTEGER')
...
SELECT
    ...
    , [d].[DatabaseName]
FROM
    [YOUR_DATABASE].[Management].[HistoryDeadLock] AS [d]
WHERE
    [d].[DatabaseName] IS NOT NULL
...
BEGIN
    ...
    (
        SELECT
            ...
            , [h].[DatabaseName]
        FROM
            [YOUR_DATABASE].[Management].[HistoryDeadLock] AS [h]
        WHERE
            [h].[DatabaseName] IS NOT NULL
...
SELECT
    ...
    , CASE DATEPART(WEEKDAY, [x].[DateDeadLock])
          WHEN 1 THEN 'Domingo'
          WHEN 2 THEN 'Segunda'
          WHEN 3 THEN 'Terça'
          WHEN 4 THEN 'Quarta'
...
CREATE OR ALTER PROCEDURE Management.sp_DeleteDeadLock
(
    ...
    BEGIN TRY
        ...
        (
            ...
            (
                ...
                FROM [YOUR_DATABASE].[Management].HistoryDeadLock AS t1
                GROUP BY CAST(t1.DateDeadLock AS DATE)
            ) AS x
        )
        ...
        BEGIN
            SET @dataMin =
            (
                SELECT CAST(DATEADD(DAY, 1, ((SELECT MIN(t1.DateDeadLock) FROM [YOUR_DATABASE].[Management].HistoryDeadLock AS t1))) AS DATE)
            )

            DELETE FROM [YOUR_DATABASE].[Management].HistoryDeadLock
            WHERE DateDeadLock < @dataMin
        ...
        SELECT @corpoFalha = @corpoFalha + '
            <table border=0 cellpadding=0 cellspacing=0 width=402 style=border-collapse: collapse;table-layout:fixed;width:1000pt;font-family:Arial;font-size:14px>
                 <tr height=20 style=height:20.0pt>
                  <td height=20 colspan=7 style=height:20.0pt;text-align:left><b>Falha na procedure [sp_DeleteDeadLock]:<b> <br>
                  </td>
                 </tr>
                 <tr height=20 style=height:20.0pt>
